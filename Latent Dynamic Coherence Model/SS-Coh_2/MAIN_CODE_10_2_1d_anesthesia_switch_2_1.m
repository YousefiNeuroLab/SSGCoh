...
plt_ml = 0;
if plt_ml==1
    ml_sscoh = [243, 350, 500, 600, 760, 800, 1100. 1215, 1502, 1590, 1592, 1592.5, 1593, 1593.5, 1600, 1600];
    ml_ldcm = [250, 380, 490, 530, 600, 610, 900. 1025, 1080, 1086, 1086, 1086.5, 1086.7, 1090.5, 1089, 1091];
    figure()
    plot(ml_ldcm,'green', 'linewidth', 4)
    hold on 
    plot(ml_sscoh,'blue', 'linewidth', 4)
    xlabel('Iteration')
    ylabel('ML Growth (max Q)')
    ylim([min(ml_sscoh)-100, max(ml_sscoh)+100])
    xlim([1, length(ml_sscoh)])
    legend('SS-Coh', 'LDCM')
    ax = gca;
    ax.FontSize = 16;
end
